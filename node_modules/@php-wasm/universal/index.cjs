...
`);const s=new Set(extractPHPFunctionsFromStack(e||""));let n=r;do{for(const i of extractPHPFunctionsFromStack(n.stack||""))s.add(i);n=n.cause}while(n);functionsMaybeMissingFromAsyncify=Array.from(s);for(const i of s)t+=`    * ${i}
`;return t+=`Original error message: ${r.message}
`,t}return r.message}const UNREACHABLE_ERROR=`
